...
def read_data_list(args, pattern):
    if pattern == 'train':
        txtfile = os.path.join(args.data_dir, 'patch_data_list_train.txt')
    else:
        txtfile = os.path.join(args.data_dir, 'patch_data_list_test.txt')
    ...
    print(f'The size of  \"{args.patch_dir}/{pattern}\" = {len(data_list)}, equal ply = {int(len(data_list)/args.patch_num)}')
    return data_list
